...
int main(int argc, char *argv[])
{
    ...
    double *pivot_row = (double *)malloc(dim * col_size * sizeof(double));
    ...
    for (int row = 0; row < dim; row++)
    {
        ...
        if (rank == row_rank)
        {
            ...
            for (int col = row; col < col_size; col++)
            {
                ...
                pivot_row[row * col_size + col] = chunk[curr_row * col_size + col]; // put into pivot row
                // so we say, the pivot row of row is chunk[curr_row * col_size + col]
            }
            // send the pivot to every other rows

            for (int i = 0; i < size; i++)
            {
                MPI_Isend(pivot_row + row * col_size, col_size, MPI_DOUBLE, i, 0,
        ...
        {
            MPI_Recv(pivot_row + row * col_size, col_size, MPI_DOUBLE, row_rank, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
            eliminate_col_from_pivot(0, rows_cnt, pivot_row + row * col_size, chunk, row, col_size);
        }
    }
